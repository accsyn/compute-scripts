''' 
	...
	Changelog:
		...
		v1r8; Compliance to accsyn v1.4.
...
class App(Common):
	...
	def usage():
		...
		Common.info("")
		Common.info("       <path_json_data>  Execute app on data provided in the JSON and ACCSYN_xxx environment variables.")
		Common.info("")
	...
	def get_commandline(self, item):
		...
		if Common.is_lin():
			...
			assert (os.path.exists(path_vray_dll)),("V-ray for Maya 2020 not properly installed! (missing: '%s')"%path_vray_dll)
			assert (os.path.getsize(path_vray_dll)==App.LIB_REF_SIZE),("V-ray for Maya v4.30.02 is not the installed version on this node (DLL size: %d bytes, should be: %d bytes)!"%os.path.getsize(path_vray_dll), App.LIB_REF_SIZE)
