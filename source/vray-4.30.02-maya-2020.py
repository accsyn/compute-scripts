...
try:
    ...
    sys.stderr.write('Cannot import accsyn common app (required), '
...
class App(Common):
    ...
    def usage():
        ...
        Common.info('   Accsyn compute app "%s" v%s-%s(common: v%s-%s) '%(
            n, v, App.__revision__, cv, Common.__revision__))
        Common.info('')
        Common.info('   Usage: python %s {--probe|<path_json_data>}'%n)
        Common.info('')
        Common.info('       --probe           Have app check if it is found and'
                    ' of correct version.')
        Common.info('')
        Common.info('       <path_json_data>  Execute app on data provided in '
    ...
    def get_commandline(self, item):
        ...
        if Common.is_lin():
            ...
            assert (os.path.exists(path_vray_dll)),('V-ray for Maya 2020 not '
                'properly installed! (missing: "%s")'%path_vray_dll)
            assert (os.path.getsize(path_vray_dll)==App.VRAY_LIB_REF_SIZE), \
                ('V-ray for Maya v%s is not the installed version on this'
                    ' node (DLL size: %d bytes, should be: %d bytes)!'%(
                        App.VRAY_VERSION, 
                        os.path.getsize(path_vray_dll), 
                        App.VRAY_LIB_REF_SIZE))
        ...
        raise Exception('This operating system is not recognized by this accsyn'
         ' app!')
